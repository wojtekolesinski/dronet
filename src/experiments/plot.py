...
def mean_std_of_metric(
    filename_format: str, ndrones: int, alg: str, seeds: list, metric: str
):
    data = []
    for seed in seeds:
...
def plot_ndrones(
    ...
    for alg in algorithms:
        ...
        for nd in ndrones:
            data_alg.append(
                mean_std_of_metric(filename_format, nd, alg, seeds, metric)[0]
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-metric", dest="metric", action="store", type=str, help="the metric to plot"
    )
    parser.add_argument(
        "-alg",
        dest="algorithms",
    ...
    plot_ndrones(
        pattern_file,
        args.number_of_drones,
        args.metric,
        args.algorithms,
        list(range(args.initial_seed, args.end_seed)),
        out_dir,
    )
