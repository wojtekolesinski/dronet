...
def coverage_distribution(filename_format: str, ndrones: int, alg_k: int, seeds: list):
...
def mean_std_of_metric(
    filename_format: str, ndrones: int, alg_k: int, seeds: list, metric: str
):
    data = []
    for seed in seeds:
        file_name = filename_format.format(ndrones, seed, alg_k)
        with open(file_name, "r") as fp:
            ...
            if metric == "ratio_delivery_generated":
                data.append(
                    ktri_0["number_of_events_to_depot"]
...
def plot_ndrones(
    ...
    for alg_k in alg_ritrasmission:
        ...
        for nd in n_drones:
            data_alg_k.append(
                mean_std_of_metric(filename_format, nd, alg_k, seeds, metric)[0]
            )
...
def set_font():
    font = {"family": "normal", "size": ALL_SIZE}
...
if __name__ == "__main__":
    ...
    pattern_file = (
        config.EXPERIMENTS_DIR + "out__" + exp_metric + "{}_seed{}_alg_{}.json"
    )
    ...
    plot_coverage_distribution(
        pattern_file,
        n_drones,
        config.SAVE_PLOT_DIR + "coverage_distribution",
        alg_exp_suffix,
        n_seeds,
        dim_area,
    )
