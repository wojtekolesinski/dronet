...
from utilities.types import Path, Point


def compute_circle_path(radius: int, center: Point) -> list[Point]:
...
def angle_between_points(p1: Point, p2: Point, p3: Point):
    # p1=d_0, p2=D, p3=d_i
...
def projection_on_line_between_points(p1: Point, p2: Point, p3: Point):
    # p1=d_0, p2=D, p3=d_i
...
class PathManager:
    ...
    def path(self, drone_id: int) -> Path:
        ...
        if config.CIRCLE_PATH:
            return self.__cirlce_path(drone_id)
    ...
    def __cirlce_path(
        ...
        if center is None:
            center = config.depot_coordinates
        if radius is None:
            radius = config.depot_communication_range - 10
    ...
    def __demo_path(self, drone_id: int) -> Path:
...
def json_to_paths(json_file_path) -> dict[int, Path]:
