...
def next_target(
    ...
    if residual_autonomy < min(range_decision) * 1.44 + euclidean_distance(
        depot_pos, cur_position
    ):
        return depot_pos, max(
            0, residual_autonomy - euclidean_distance(cur_position, depot_pos)
        )
    else:
        feasible_positions = [
            d
            for d in range_decision
            if d * 1.44 * 2 + euclidean_distance(cur_position, depot_pos)
            <= residual_autonomy
        ]
        ...
        next_point_x = random_generator.randint(
            max(0, cur_position[0] - d), min(cur_position[0] + d, edge_area)
        )
        next_point_y = random_generator.randint(
            max(0, cur_position[1] - d), min(cur_position[1] + d, edge_area)
        )
...
def get_tour(
    ...
    if random_starting_point:
        start_point = (
            random_generator.randint(0, edge_area),
            random_generator.randint(0, edge_area),
        )
    else:
    ...
    while residual_aut >= min(range_decision) * 1.44 + euclidean_distance(
        current_point, start_point
    ):

        next_p, residual_aut = next_target(
            depot_pos,
            current_point,
            residual_aut,
            edge_area,
...
def random_waypoint_tour(
    ndrones, nrounds, depot, autonomy, edge_area, random_generator
):
    drones_tours = {}
    for d in range(ndrones):
        ...
        for r in range(nrounds):
            d_tours.append(
                get_tour(autonomy, edge_area, depot, random_generator=random_generator)
            )
...
def to_json(tours, mission_data, seed):
    ...
    for i in range(len(_tours)):
        ...
        out_json["drones"].append({"index": str(i), "tour": str_tour})
...
def run(
    ...
    # get tours
    tours = random_waypoint_tour(
        ndrones, nrounds, depot, autonomy, edge_area, random_generator=random_generator
    )
