...
class QMR(BASE_routing):
    ...
    def __init__(self, drone, simulator):
        ...
        self.delay_history = np.zeros(
            shape=(self.simulator.n_drones, self.look_back)
        )  # history delay used for normalized one-hop delay
    ...
    def feedback(self, drone, id_event, delay, outcome, reward, E_j, hop_delay):
        ...
        if id_event in self.taken_actions:
            ...
            if hop_delay is not None:
                ...
                normalized_one_hop_delay, variance = self.get_normalized_one_hop_delay(
                    action, self.delay_history[drone.identifier, 0]
                )
    ...
    def get_normalized_one_hop_delay(self, hop, delay):
        ...
        one_hop_delay = (
            0.3
            if var <= 0.1
            else (delay - np.mean(history[: np.where(history == 0)[0][0]])) / var
        )
    ...
    def relay_selection(self, opt_neighbors: list, packet):
        ...
        if len(candidate_neighbors) > 0:
            ...
            valid_q_values = self.qtable[neighbors_ids]
        ...
        if action is not None:
            self.taken_actions[packet.event_ref.identifier] = (
                ...
                [drone.identifier for hp, drone in opt_neighbors],
